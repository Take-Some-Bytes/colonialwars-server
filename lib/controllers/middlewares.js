...
class Middlewares {
  ...
  sysCheckpoint (implementedMethods) {
    ...
    return (req, res, next) => {
      ...
      const serverLogger = this.loggers.get('Server-logger')
      const securityLogger = this.loggers.get('Security-logger')
      ...
      if (reqUrlLength > this.URL_MAX_LEN) {
        ...
        errSender.sendErrorAndLog(
          'URL too long.', securityLogger, { logMsg: logMsg, status: 414 }
        )
        return
