...
class Controllers {
    ...
    return async (req, res) => {
      ...
      if (!req.query || !(req.query instanceof URLSearchParams)) {
        ...
        errSender.sendErrorAndLog(
          'Query string is required!', 'Server-logger', logMsg, 400
        )
        return
      }
      /**
      ...
      if (await db.has(data.playerName)) {
        ...
        errSender.sendErrorAndLog(
          'Player already exists.', 'Server-logger', logMsg, 409
        )
        return
        ...
        errSender.sendErrorAndLog(
          'Query string missing fields.', 'Server-logger', logMsg, 400
        )
        return
      }
  ...
  unhandled () {
    return (req, res) => {
      ...
      new ErrorSender({
        request: req, response: res, loggers: this.loggers
      }).sendErrorAndLog('404 Not Found', 'Server-logger', logMsg, 404)
    }
  }
}
