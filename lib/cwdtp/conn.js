...
export default class WSConn extends events.EventEmitter {
  ...
  _onMessage (buf, isBinary) {
    ...
    if (isBinary) {
      ...
      this.terminate(1003, 'Unexpected binary message')
      this.state = WSConnState.ERROR
    ...
    try {
      ...
      this.terminate(1007, 'Invalid CWDTP message!')
      this.state = WSConnState.ERROR
  ...
  _serverHandshake () {
    ...
    this._ws.on('message', async function onMsg (buf, isBinary) {
      ...
      if (isBinary) {
        ...
        self.emit('error', new errors.InvalidMsgError(
          'Unexpected binary message!',
          errors.InvalidMsgErrorCode.UNEXPECTED_BINARY
        ))

        self.terminate(1003, 'Unexpected binary message')
        self.state = WSConnState.ERROR
