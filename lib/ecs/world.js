...
import { Uint32ArrUtils } from '../utils/array-utils.js'
import SparseSet from './sparse-set.js'
import WorldQuery from './query.js'

const debug = debugFactory('ecs:world')
...
export default class World {
  ...
  create () {
    ...
    if (idx < this._entities.length) {
      debug('Recycling entity at index %d', idx)
    ...
    debug('Creating new entity at index %d', idx)
  ...
  destroy (entity) {
    ...
    debug('Deleting entity at index %d', idx)
  ...
  registerComponent (name, componentClass) {
    ...
    debug('Registering new component "%s"', name)
  ...
  deregisterComponent (name) {
    ...
    debug('Deregistering component "%s"', name)
  ...
  addComponent (name, opts) {
    ...
    debug('Adding/overriding component "%s" for entity at index %d', name, Entity.getIndex(entity))
  ...
  getComponent (name, opts) {
    ...
    debug('Getting component "%s" from entity at index %d', name, Entity.getIndex(entity))
    ...
    debug('Component "%s" entry for entity %d: %o', name, Entity.getIndex(entity), entry)
  ...
  removeComponent (name, opts) {
    ...
    debug('Removing component "%s" from entity at index %d', name, Entity.getIndex(entity))
  ...
  removeAllComponentsOf (entity) {
    ...
    debug('Removing all components of entity at index %d', idx)
  ...
  * allWith (name) {
    ...
    debug('Iterating over all entities with component "%s"', name)
  ...
  /**
   * Returns an iterator that iterates over all the instances of the specified
   * component and the entity associated with it.
   * @param {string} component The name of the component.
   * @returns {Generator<ComponentEntry<unknown>, void, void>}
   */
  * allInstancesOf (component) {
    if (!this._components.has(component)) {
      throw new Error('Component does not exist!')
    }

    debug('Iterating over all instances of component "%s"', component)

    yield * this._components.get(component).current.values()
  }
  ...
  * allComponentsOf (entity) {
    ...
    debug('Getting all components of entity at index %d', idx)
  ...
  /**
   * Query the entities in this world.
   * @returns {WorldQuery}
   */
  query () {
