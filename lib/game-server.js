...
class GameServer {
  ...
  constructor (opts) {
    const {
      ...
      gamelogger, authStore, allowedOrigins
    } = opts
  ...
  _onConnection (conn, req) {
    ...
    const readyTimeout = setTimeout(() => {
      ...
      conn.disconnect(true, 4004, 'Timeout')
    }, 10000)
    ...
    conn.on(communications.CONN_READY, () => {
      ...
      game.addNewPlayer(conn, {
        name: data.name,
        team: data.team
      })

      conn.emit(communications.CONN_READY_ACK, {
        worldBounds: game.worldLimits,
        mapTheme: game.mapTheme,
        static: {}
      })
    ...
    conn.on(communications.CONN_CLIENT_ACTION, input => {
      const player = game.getPlayerByConnID(conn.id)
      if (!player) {
