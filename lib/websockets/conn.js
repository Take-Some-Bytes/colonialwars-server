...
class WSConn extends events.EventEmitter {
  ...
  /**
   * Attaches the required event listeners.
   * @private
   */
  ...
  doServerHandshake () {
    ...
    debug('Waiting for client handshake message...')
    // Wait.
    let handshakeTimeout = setTimeout(() => {
      this.disconnect(true, 4002, 'Handshake timed out', true)
      debug('Handshake timed out!')
    ...
    this.on('cwdtp::client-hello', async msg => {
      ...
      clearTimeout(handshakeTimeout)
      handshakeTimeout = null
      debug('Client handshake message received.')
      debug('Request key: %s', msg.meta.req_key)
