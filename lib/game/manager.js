 ...
 * @prop {InstanceType<(id: string) => import('./player')>} getPlayerByID
 * @prop {(conn: WSConnInstance, meta: PlayerMeta) => void} addPlayer
 * @prop {(conn: WSConnInstance, reason?: string, sendReason?: boolean) => void} removePlayer
 */
  ...
  getGame (gameID) {
    ...
    if (this.games.has(gameID)) {
      return {
        ...
        removePlayer: (...args) => {
          this.removeClientFrom(gameID, ...args)
        },
        clearPlayers: () => {
