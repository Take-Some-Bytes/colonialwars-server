...
class BaseGame {
   ...
   * @param {string} id A unique ID for this game instance.
   * @param {string} name A human-readable name for this game.
   * @param {string} mode The game mode that this game is in. Purely informational.
   * @param {number} maxPlayers A number representing the maximum number of players this
   * game is allowed to accept.
   * @param {WorldLimits} worldLimits The X and Y limits of the game world.
   * @param {StartPositions} startPositions The start positions of each team's players.
   * @param {BaseGameConfig} config Configurations.
   */
  constructor (id, name, mode, maxPlayers, worldLimits, startPositions, config) {
    const {
   ...
   * @param {string} message A message describing when the player has been removed.
   */
  ...
  sendState () {
    ...
    this.clients.forEach((client, id) => {
      ...
      if (this.numEmits > 25 * 30) {
        this.debug('Packet size: ', Buffer.from(state, 'utf-8').byteLength)
        this.numEmits = 0
      }
