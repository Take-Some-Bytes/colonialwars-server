...
class Player extends BoundEntity {
  ...
  constructor (config) {
    const {
      name, team, socketID, position, debug,
    ...
    // debug.Debugger for development.
    this.debug = debug
  ...
  addInputToQueue (data) {
    if (
      ...
      // Reject it.
      this.debug('Received invalid timestamp for input #%d', data.inputNum)
  ...
  processInputs () {
    ...
    while ((nextInput = inputs.shift())) {
      ...
      deltaTime = input.timestamp - this.lastUpdateTime
