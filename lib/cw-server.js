...
class CWServer {
  ...
  async _initGameHelpers () {
    this.gameloader = new GameLoader({
      ...
      baseDir: this.config.GAME_CONF_BASE_DIR,
      gameConstants: {
        playerStats: { PLAYER_SPEED: this.config.PLAYER_SPEED },
        communications: constants.COMMUNICATIONS
      }
    })
    ...
    this.manager = await Manager.create({
      ...
      gameConfs: this.config.GAME_CONFS,
  ...
  _initApp () {
    ...
    this.router.use(
      /**
       * TODO: Check if cookies are needed.
